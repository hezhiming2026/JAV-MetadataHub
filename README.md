...
R18 当前实现不支持真实 `.sql` / `.sql.gz` dump。FANZA 当前实现只把 raw responses 写入
`source_records`，并通过 parser / ingestion 写入 `field_observations`；不做 canonical promotion。
